...
import 'package:news_app/Model/HealthNewsModelClass.dart';
...
class _HealthNewsScreenState extends State<HealthNewsScreen> {
  Future<HealthNewsModelClass> getHealthNews() async {
    String url = "https://newsapi.org/v2/top-headlines?country=in&category=health&apiKey=090c008511454296a2e233a6b2e42433";
    final response = await http.get(Uri.parse(url));
    var data = jsonDecode(response.body.toString());
    if (response.statusCode == 200) {
      return HealthNewsModelClass.fromJson(data);
    } else {
      return HealthNewsModelClass.fromJson(data);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(
          children: [
            Text("Health ",style: TextStyle(fontWeight: FontWeight.bold,letterSpacing: 1,color: Colors.white,fontSize: 25.sp),),
            Text("NEWS",style: TextStyle(fontWeight: FontWeight.bold,letterSpacing: 1,color:Colors.blueAccent,fontSize: 25.sp),),
          ],
        ),),
      body: FutureBuilder<HealthNewsModelClass>(
