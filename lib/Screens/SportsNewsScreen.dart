...
import 'package:news_app/Model/sportsNewsModelClass.dart';
import 'package:url_launcher/url_launcher.dart';


class SportstNewsScreen extends StatefulWidget {
  const SportstNewsScreen({Key? key}) : super(key: key);
  @override
  State<SportstNewsScreen> createState() => _SportstNewsScreenState();
}


class _SportstNewsScreenState extends State<SportstNewsScreen> {
  Future<SportsNewsModelClass> getSportsNews() async {
    String url = "https://newsapi.org/v2/top-headlines?country=in&category=sports&apiKey=090c008511454296a2e233a6b2e42433";
    final response = await http.get(Uri.parse(url));
    var data = jsonDecode(response.body.toString());
    if (response.statusCode == 200) {
      return SportsNewsModelClass.fromJson(data);
    } else {
      return SportsNewsModelClass.fromJson(data);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(
          children: [
            Text("Sports ",style: TextStyle(fontWeight: FontWeight.bold,letterSpacing: 1,color: Colors.white,fontSize: 25.sp),),
            Text("NEWS",style: TextStyle(fontWeight: FontWeight.bold,letterSpacing: 1,color:Colors.blueAccent,fontSize: 25.sp),),
          ],
        ),),
      body: FutureBuilder<SportsNewsModelClass>(
